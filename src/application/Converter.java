...
public class Converter
{
	...
	public void applyMeasureType() throws ParseException
	{
		...
		Set<Unit<Double>> units = UnitsFactory.getUnits(measureType.get());
		for(Unit<Double> obj : units) {
			if (obj.isSetable()) {
				sourceUnits.add(obj);
			}
			destinationUnits.add(obj);
		}
			...
			if (!input.isEmpty()) {
				...
				if (parsed){
					...
					try {
						...
						logger.severe(" Conversion not possible with " + e.getLocalizedMessage());
					}
				}
			}
		}
	...
	public void convert() throws IllegalStateException
	{
		...
		Unit<Double> source = sourceUnit.get();
		if(source == null) {
			throw new IllegalStateException("source est null");
		}
		if (!source.hasValue()) {
			throw new IllegalStateException("source n'a pas de valeur");
		}
	...
	public void switchUnits()
	{
		...
		Unit<Double> source = sourceUnit.get();
		if (source == null) {
			throw new NullPointerException("source unit est null");
		}
		
		Unit<Double> destination = destinationUnit.get();
		if (destination == null) {
			throw new NullPointerException("destination unit est null");
		}		
		
		if(!unexchangeableUnits.get()) {
			setSourceUnit(destination);
			setDestinationUnit(source);
		}
		else {
			logger.warning("Pas possible de switch");
		}
	}
