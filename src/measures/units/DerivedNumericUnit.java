...
public class DerivedNumericUnit extends BoundedBaseNumericUnit
{
	...
	/**
	 * The Factor to apply to values in SI unit to convert to this unit
	 */
	    ...
	    IllegalArgumentException
	{
		...
		/*
		 * DONE Assert |power| is not lower than 1e-6 otherwise throw IllegalArgumentException
		 */
		
		double power_abs = Math.abs(power);
		if (power_abs < 1e-6) { 
			throw new IllegalArgumentException("Power est trop faible");
		}
		
		/*
		 * DONE Assert |factor| is not lower than 1e-6 otherwise throw IllegalArgumentException
		 */
		
		double factor_abs = Math.abs(factor);
		if ( factor_abs < 1e-7) { // Pour les millimetres on a factor = 1e-6 ce qui pose problème
			throw new IllegalArgumentException("Factor est trop faible");
		}
	...
	public double getSIValue() throws NoSuchElementException
	{
		...
		/*
		 * DONE Use OperationOrder order operations to return correct SI value
		 */
		
		if (!hasValue()) {
			throw new NoSuchElementException("pas de value à convertir");
		}
		
		else {
			double valeur = value.get(); 
			return order.toSI(valeur, factor, offset, power);
		}
	...
	public void convertValueFrom(Unit<?> unit)
	{
		...
		/*
		 * DONE Convert from SI value using:
		 * 	- the SI value of "unit"
		 * 	- converted using OperationOrder order operations to
		 * 	- setValue_Impl
		 */
		double SIvaleur = unit.getSIValue();
		double SIValue_converted = order.fromSI(SIvaleur, factor, offset, power);
		setValue_Impl(SIValue_converted);
	}
}
