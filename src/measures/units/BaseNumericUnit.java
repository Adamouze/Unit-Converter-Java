...
public abstract class BaseNumericUnit extends Unit<Double>
{
	...
	{
		// DONE complete BaseNumericUnit::getSIValue();
		if (hasValue()) {
			return getValue();
		} else {
			throw new NoSuchElementException("rawUnit n'a pas de valeur");
		}
	}
	...
	{
		// DONE Complete BaseNumericUnit::formatValue() using formatter
		if (formatter==null) {
			throw new NullPointerException("BaseNumericUnit : formatter est nulle");
		}
	
		if (!hasValue()) {
			throw new NoSuchElementException("BaseNumericUnit : formatter n'a pas de valeur");
		}
	...
	public void convertValueFrom(Unit<?> unit)
	{
		...
		// DONE Complete BaseNumericUnit::convertValueFrom(Unit<?> unit) using unit SI value
		setValue_Impl(unit.getSIValue());
