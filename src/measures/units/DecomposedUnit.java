...
public class DecomposedUnit extends Unit<Double>
{
	...
	{
		/*
		 * DONE use #rawUnit to determine if we have a Value
		 */
	...
	{
		if (!hasValue()) {
			throw new NoSuchElementException("Il n'y a pas de valeur");
		}
	...
	{
		/*
		 * DONE use #rawUnit to get SI value
		 */
		if (!hasValue()) {
			throw new NoSuchElementException("Il n'y a pas de valeur SI");
		}
	 ...
	 */
	@Override
	public void convertValueFrom(Unit<?> unit) throws IllegalStateException
	{
		/*
		 * DONE use #rawUnit to convert value from unit
		 */
		if (!unit.hasValue()) {
			throw new IllegalStateException("Provided Unit n'a pas de valeur");
		}
		else {
			rawUnit.convertValueFrom(unit); //TODO
		}
	}
}
