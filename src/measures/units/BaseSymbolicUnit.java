...
public class BaseSymbolicUnit extends BoundedBaseNumericUnit
{
	    ...
	    throws NullPointerException, IllegalArgumentException, ParseException
	{
		...
		/*
		 * DONE Assert Values contains increasing values (otherwise throw IllegalArgumentException)
		 */
		
		Double precedent = values[0];
		for(int i=1; i<values.length;i++) {
			Double suivant = values[i];
			if (suivant < precedent) {
				throw new IllegalArgumentException("not increasing values");
			}
			precedent = suivant;
		}

		/*
		 * DONE Assert symbols have exactly 1 more elt than values (otherwise throw IllegalArgumentException)
		 */
		
		if (symbols.length > values.length + 1) {
			throw new IllegalArgumentException("incorrect lenght");
		}
